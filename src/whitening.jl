...
function whiten_mmap(
    ...
    w_sz = size(w)
    w_sz == (nx, nx) || throw(ArgumentError("Size of covariance matrix does not match"))
    ...
    R = promote_type(T, S)
    outs = Vector{Vector{R}}(undef, nx)
    paths = Vector{String}(undef, nx)
    if isempty(fids)
        map!(
            (::Any) -> joinpath(
                basedir,
                string(basename(tempname()), suffix, '_', R, '_', nel, ".dat"),
            ),
            paths,
            1:nx,
        )
        ...
        map!(
            fid -> joinpath(basedir, string(fid, suffix, '_', R, '_', nel, ".dat")),
            paths,
            fids,
        )
    end
    ...
    for i = 1:nx
        outs[i], paths[i] =
            typemmap(Vector{R}, (nel,); fpath = paths[i], autoclean = autoclean)
    end
...
function _whiten_mmap!(
    ...
    means = Vector{div_type(T)}(undef, nx)
    scratch = Vector{R}(undef, nx)
    whitened = Vector{R}(undef, nx)
    means .= mean.(xs)
    for elno = 1:nel
        @simd for xno = 1:nx
